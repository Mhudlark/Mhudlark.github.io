	...
	return (
		<div className="about-container">
			...
			<div className="previous-experience-container">
				<h3 className="h-400 subtitle">
					{aboutContent.previousExperience.subtitle}
				</h3>
				<ul className="experiences-container">
					{aboutContent.previousExperience.experiences.map(
						(experience, index) => (
									...
									{experience.roles.map((role, index) => (
										<li className="role" key={index}>
											<div className="date-title-container">
												<div className="p role-date">
													{role.dates}
												</div>
												<div className="p role-title">
													{role.organisationAndRole}
												</div>
											</div>
											{Array.isArray(role.dotPoints) &&
												role.dotPoints.length > 0 && (
													<ul className="description-points-container">
														{role.dotPoints.map(
															(point, index) => (
																<li
																	className="p description-point"
																	key={index}
																>
																	{point}
																</li>
